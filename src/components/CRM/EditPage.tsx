import {
  ...
  import { fb_getPageContent, fb_getPostById, fb_savePageContent, fb_savePost } from "@/lib/firebase";
  ...
  const EditPage: React.FC<EditPageProps> = ({ pageId, pageLang }) => {
    ...
    useEffect(() => {
      fb_getPageContent(firebaseApp, pageId, pageLang)
      .then((pageContent) => {
        if (pageContent) {
          setPageData({
            content: pageContent
    ...
    const formSubmitHandler: SubmitHandler<FormData> = async (data) => {
      ...
      try {
        await fb_savePageContent(firebaseApp, pageId, pageLang, data.content);
